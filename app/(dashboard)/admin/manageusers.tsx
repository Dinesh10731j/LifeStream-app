...
import { ScrollView, View, TouchableOpacity, Text, StyleSheet, Modal, Button } from 'react-native';
...
const ManageUsers = () => {
  ...
  const handleSaveRole = () => {
    console.log('Updated Role:', selectedRole, 'for User:', selectedUser?.name);
  ...
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ flexGrow: 1, height: '100%', backgroundColor: 'white' }}>
      <View>
        ...
        <ScrollView style={styles.dataWrapper}>
          <Table borderStyle={{ borderWidth: 1, borderColor: '#C1C0B9' }}>
            {tableData.map((rowData: (string | (() => JSX.Element))[], index: number) => (
              <Row
                key={index}
                ...
                widthArr={widthArr}
                style={[styles.row, index % 2 ? { backgroundColor: '#F7F6E7' } : null]}
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalView}>
            <Text style={styles.modalText}>Edit Role for {selectedUser?.name}</Text>
            <Picker
              selectedValue={selectedRole}
              onValueChange={(itemValue) => setSelectedRole(itemValue)}
            >
              <Picker.Item label="User" value="donor" />
              <Picker.Item label="Admin" value="admin" />
              <Picker.Item label="Manager" value="receiptant" />
            </Picker>
            <Button title="Save" onPress={handleSaveRole} />
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalView}>
            ...
            <Link href={{ pathname: '/userhistory/[id]', params: { id: selectedUser?.email ? bs64.encode(selectedUser.email) : '' } }}>
             View History
            </Link>
...
const styles = StyleSheet.create({
  ...
  modalView: { width: 300, padding: 20, backgroundColor: 'white', borderRadius: 10 },
